...
/
 ...
 CREATE SEQUENCE   "RP_SERVICE_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 2644 CACHE 20 NOORDER  NOCYCLE
/
...
/
 CREATE SEQUENCE   "PRE_RP_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 2548 CACHE 20 NOORDER  NOCYCLE
/
 CREATE SEQUENCE   "PRE_REVOLUT_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 148 CACHE 20 NOORDER  NOCYCLE
/
...
/
 CREATE SEQUENCE   "CONTRAGENTS_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 2277 CACHE 20 NOORDER  NOCYCLE
/
...
CREATE OR REPLACE FORCE VIEW  "V_CARD_TO_AUTH" ("reference", "ID", "value_date", "TR_DATE12", "contragent", "TERM_NAME_LOC", "debit", "TR_AM", "CH_AM") AS 
  	"reference"	,ID	,"value_date"	,TR_DATE12	,"contragent"	,TERM_NAME_LOC	,"debit"	,TR_AM	,CH_AMFROM CC_CREDIT_CARD ccc, CC_CARD_AUTHORIZATIONS caWHERE "contragent" LIKE TERM_NAME_LOC || '%'AND "value_date" = TR_DATE12AND "debit" = CH_AMORDER BY "value_date" DESC, TR_DATE12 DESC
/
CREATE OR REPLACE FORCE VIEW  "V_CC_DST_FROM_AVERAGE" ("YEAR", "MONTH", "DST_DEBIT", "DST_CREDIT", "DST_BALANCE") AS 
  	ccc.YEAR_CC AS "YEAR"	,ccc.MONTH_CC AS "MONTH"	,ROUND((SUM("debit") - (SELECT AVG_DEBIT FROM V_CC_MONTHLY_STATS vcms WHERE VCMS."MONTH" = ccc.MONTH_CC)) / (SELECT STD_DEBIT FROM V_CC_MONTHLY_STATS vcms WHERE VCMS."MONTH" = ccc.MONTH_CC),2) AS DST_DEBIT	,ROUND((SUM("credit") - (SELECT AVG_CREDIT FROM V_CC_MONTHLY_STATS vcms WHERE VCMS."MONTH" = ccc.MONTH_CC)) / (SELECT STD_CREDIT FROM V_CC_MONTHLY_STATS vcms WHERE VCMS."MONTH" = ccc.MONTH_CC),2) AS DST_CREDIT	,ROUND(((SUM("debit") - SUM("credit")) - (SELECT AVG_BALANCE FROM V_CC_MONTHLY_STATS vcms WHERE VCMS."MONTH" = ccc.MONTH_CC)) / (SELECT STD_BALANCE FROM V_CC_MONTHLY_STATS vcms WHERE VCMS."MONTH" = ccc.MONTH_CC),2) AS DST_BALANCEFROM FINANCE.CC_CREDIT_CARD cccGROUP BY ccc.YEAR_CC, ccc.MONTH_CCORDER BY ccc.YEAR_CC DESC, ccc.MONTH_CC
/
CREATE OR REPLACE FORCE VIEW  "V_CC_MONTHLY_STATS" ("MONTH", "AVG_DEBIT", "AVG_CREDIT", "AVG_BALANCE", "STD_DEBIT", "STD_CREDIT", "STD_BALANCE", "MIN_CREDIT", "MIN_DEBIT", "MIN_BALANCE", "MAX_CREDIT", "MAX_DEBIT", "MAX_BALANCE") AS 
  	"MONTH"	,ROUND(AVG(DEBIT),3) AS AVG_DEBIT	,ROUND(AVG(CREDIT),3) AS AVG_CREDIT	,ROUND(AVG(BALANCE),3) AS AVG_BALANCE	,ROUND(STDDEV(DEBIT),3) AS STD_DEBIT	,ROUND(STDDEV(CREDIT),3) AS STD_CREDIT	,ROUND(STDDEV(BALANCE),3) AS STD_BALANCE	,ROUND(MIN(CREDIT),3) AS MIN_CREDIT	,ROUND(MIN(DEBIT),3) AS MIN_DEBIT	,ROUND(MIN(BALANCE),3) AS MIN_BALANCE	,ROUND(MAX(CREDIT),3) AS MAX_CREDIT	,ROUND(MAX(DEBIT),3) AS MAX_DEBIT	,ROUND(MAX(BALANCE),3) AS MAX_BALANCEFROM 	(		SELECT 			YEAR_CC AS "YEAR"			,MONTH_CC AS "MONTH"			,SUM("debit") AS DEBIT			,SUM("credit") AS CREDIT			,SUM("debit") - SUM("credit") AS BALANCE		FROM CC_CREDIT_CARD ccc		GROUP BY YEAR_CC, MONTH_CC		ORDER BY YEAR_CC DESC, MONTH_CC	)GROUP BY "MONTH"ORDER BY "MONTH"
/
