DROP TABLE IF EXISTS  "ENTRANCE" cascade constraints
/

DROP TABLE IF EXISTS  "HA" cascade constraints
/
DROP TABLE IF EXISTS  "BUILDING" cascade constraints
/
drop TABLE "APARTMENTS" cascade constraints
/

DROP TABLE IF EXISTS  "EXPENSE_50" cascade constraints
/

DROP TABLE IF EXISTS  "APEX$_ACL" cascade constraints
/

DROP TABLE IF EXISTS  "APEX$_WS_ROWS" cascade constraints
/

DROP TABLE IF EXISTS  "APEX$_WS_FILES" cascade constraints
/

DROP TABLE IF EXISTS  "APEX$_WS_HISTORY" cascade constraints
/

DROP TABLE IF EXISTS  "APEX$_WS_LINKS" cascade constraints
/

DROP TABLE IF EXISTS  "APEX$_WS_NOTES" cascade constraints
/
...
/
DROP TABLE IF EXISTS  "CALC_METHOD" 
...
/
DROP TABLE IF EXISTS  "EXPENSES_PRE" 
   (	"ID" NUMBER, 
	"ГОДИНА" NUMBER, 
	"МЕСЕЦ" NUMBER, 
	"ДАТА" DATE, 
...
/
DROP TABLE IF EXISTS  "INCOME_LOG_PRE" 
   (	"ID" NUMBER, 
	"ГОДИНА" NUMBER, 
	"МЕСЕЦ" NUMBER, 
	"ЕТ" NUMBER, 
...
/
CREATE OR REPLACE TRIGGER  "bi_EXPENSES_PRE"   before insert on "EXPENSES_PRE"                for each row   if :new."ID" is null then    select "EXPENSES_PRE_SEQ".nextval into :new."ID" from dual;  end if;end;
/
...
/
CREATE OR REPLACE TRIGGER  "HABITANTS_HOUSEHOLDID" BEFOREinsert on "HABITANTS"for each rowbegin
